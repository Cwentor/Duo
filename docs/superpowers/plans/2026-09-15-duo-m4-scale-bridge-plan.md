...
## 附录：M4 关闭后的补充与勘误（2026-09-18）

M4 已于 2026-09-15 关闭；2026-09-18 的独立复验与其后的处置对本计划有三处修正，实现以修正后的语义为准：

1. **[HIGH，`317e9af`] T38 容器档的 `restart` 语义收窄**：原计划 T38 未讨论容器档 `restart()`。
   `ZkBackedRegistry.restart()`＝stop→start，而容器重启会换宿主端口，与 D2「wire 客户端复用旧端口」冲突
   → 永久挂起且无失败路径。修订：`ZookeeperContainerRegistry` 覆写 `restart()` 显式抛
   `UnsupportedOperationException`（§7.2 无降级）。**若将来要支持容器档 restart/flap，须先引入固定
   `PortBinding` 再覆写**（见独立复验记录 §6「未做」）。
2. **[MEDIUM，`317e9af`] T38 判据补「守卫脱离 Docker 门控」**：原计划判据为「无 Docker 时 4 条测试 skip
   且构建绿」，但守卫类用例若一并被 `@EnabledIf(dockerAvailable)` 吃掉，「不支持＝显式拒绝」这一安全属性
   在无 Docker 环境无人验证。修订：新增 `ZookeeperContainerRegistryGuardTest`（5 条，**不标** `@EnabledIf`）。
3. **[MEDIUM，`6d44ccc`] §3 的验收口径下沉到 M3 验收测试**：`ControlPlaneAcceptanceTest` 的注入点由
   `sleep(1000)` 改为断言式等待（`awaitInFlightTaskOnWorkers2`）。此项属 M3 交付物，因 M4 压测暴露的
   时序敏感问题一并整改，记录在此以免 M4 计划读者以为「注入点即固定延时」是约定。

> 本计划 §3 任务表、§4 D1~D6 与上述修正冲突时，以后者为准；全仓测试数随改动变化，
...
- **v1.1（2026-09-18）**：补本文档缺失的**修订记录章节**（M0~M3 各计划均设此章节，M4 计划初稿遗漏）+ 上述
  三处「关闭后补充与勘误」；其中第 1、2 条源自 M4 独立复验的两项需处置项（`317e9af`），第 3 条为同批
  回归中整改的 M3 交付物（`6d44ccc`）。**未改动 §1~§4 的原始承诺文本**，仅追加勘误。
