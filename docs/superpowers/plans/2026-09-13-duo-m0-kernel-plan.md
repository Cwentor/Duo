...
- 运行内容：demo-scheduler 执行一个带依赖的小型 DAG（如 5 个任务），其中**一个具名任务（如 `unstable-task`）经精确名 match 绑定到含 `exception` 的具名 profile（每次尝试必失败），由调度器有界重试后进入 FAILED 终态**；其余任务走 default profile 的成功路径——**成功与失败两条路径均被确定性覆盖（选择性与确定性同时成立，不依赖概率字段）**
...
| T2 | `duo-sim-protocol` | 帧格式（magic + version + length 前缀）、`FrameCodec`、消息集：worker 契约 `Register/Heartbeat/SlotReport/TaskDispatch/TaskAck/TaskCancel/TaskStatus`（下行含取消报文，与 §2 连接模型对齐；**scheduler 侧主动取消留 M1，M0 由编解码 round-trip 与 worker 端 CANCELLED 回报单测覆盖**；scheduler 契约 M0 复用同一组报文的 server 侧语义）；**连接模型按 §2 钉死：worker 拨号、单条双向长连接、帧双向流动** | 编解码 round-trip 单测（含 TaskCancel）、畸形帧拒绝单测、双向帧交织单测 |
...
| T8 | VirtualWorker | 每 Instance 一个虚拟线程；心跳 + 槽位上报 + 任务执行（TaskStub）+ 状态回报（沿 §2 连接模型：拨号 master、单长连接）；`InstanceControl` 实现；自身 DUO_PORT 监听（就绪探测用）；**发现等待语义：启动后查询 registry，未发现 master 时按固定参数重试（500ms × 20 次，超限按 §12 启动失败）——workers 与 master 启动顺序不定（均仅依赖 zk），闭环不得依赖时序** | 单测：心跳节律、实例级 stop/restart（端点身份保留）、发现（含先于 master 就绪的等待重试）→连接→派发→回报闭环、收到 TaskCancel 回报 CANCELLED |
...
5. **发现链路的单点依赖与时序**：M0 验收依赖"master 端点写入 registry"这一步——既怕 demo-scheduler 忘记注册（单点），也怕 workers 先于 master 就绪就去查询（时序）。对策：T13 完成判据含"注册事实事件"；T8 发现等待重试语义（500ms × 20 次，超限启动失败）使闭环不依赖启动顺序；T15 验收失败时优先检查该链路。
...
- **v2（2026-09-13）**：依计划评审修订——
  ...
  4. **[P2] master 端点发现钉死为 registry 路径**：workers wiring 仅含 registry 槽，master 端点写临时节点；**计划 §2 内注明**设计文档 §8 示例的 workers→scheduler 槽为 DSL 能力演示、M0 验收不使用（spec 冻结未动）；
...
- **v3（2026-09-13）**：依 v2 计划评审修订——
  1. **[P1] 绑定子集升级（采建议方案①）**：M0 绑定由"仅 default"升级为"default + 一个具名 profile + 精确名 match"，匹配优先级实现两级（精确任务名 > default；标签/通配留 M1 全量优先级）——§1 验收的"一个具名任务失败、其余成功"得以确定性表达，选择性与确定性同时成立，且预铺 M1 金标准的 match 绑定（§1/T11 同步）；
  2. **[P3] T2 消息集补 `TaskCancel`**：下行取消报文与 §2 连接模型对齐；scheduler 侧主动取消留 M1，M0 由 round-trip 与 worker 端 CANCELLED 回报单测覆盖；
  3. **[P3] T4 消费方清单补全**：+T6（VirtualRegistry 实现 RegistryContract）、+T14（demo real worker 实现 WorkerContract）；
  4. **[P3] 发现等待语义钉死**：T8 明确 500ms × 20 次重试、超限按 §12 启动失败；风险 5 扩展覆盖启动时序；
  5. **[P3] 措辞修正**：v2 修订记录第 4 条改为"计划内注明"（spec 冻结未动）。
