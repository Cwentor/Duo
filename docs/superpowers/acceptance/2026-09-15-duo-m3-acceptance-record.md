...
> **口径说明（2026-09-18 补，回应独立验收报告 LOW-4）**：上表是 **M3 关闭时刻（`abdfc44`/`74dd0b1` 一代）
> 的提交快照**，不是「可复算到任意 HEAD」的不变量。此后每次实质性改动都会改变测试数：
> `7ac458b` 为两处缺陷补 3 条 kernel 回归 + 1 条 control 用例 → **219**（独立验收报告 2026-09-18 实测）；
> `ed2f054`（M4 T38）再增用例 → **221**（本机 2026-09-18 实测，与 M4 验收记录 §五一致）。
> 表中行值本身自洽（9+54+31+42+30+0+0+47 = 213），历史合计值**不应**再按当前 HEAD 复算。
> 后续记录统一采用「合计（快照语义）+ 对应提交」的写法。
