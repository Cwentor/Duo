...
| `sut` | `SutLauncher`（in-process：独立虚拟线程调阻塞 `run()`、ready 握手、协作式停止、退出/崩溃事件）；`ExternalSutLauncher`（external：写端点配置文件 + 代起子进程 + stdout 端点兜底 + 轮询 ready + 退出观测 + 启动失败销毁）；`ReadyProbe`（`tcp`/`http` 探针规格与单次探测） |
